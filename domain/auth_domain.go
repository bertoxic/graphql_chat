...
import (
	...
	"github.com/bertoxic/graphql_chat/utils"
...
func (as *AuthService) Register(ctx context.Context, input packages.RegistrationInput) (*AuthResponse, error) {
	// Validate input
	...
	exists, err := as.userRepo.UserExists(ctx, input.Email, input.Username)
	if err != nil {
		return nil, err
	}
	if exists {
...
func validateRegistrationInput(input packages.RegistrationInput) error {
